...
#多线程执行
def run_model(model, samples_target, label_source, source_only, samples_source, mem_fea, mem_cls, class_weight_src, img_idx, source_idx, result_list):
    total_loss = model(samples_target, label_source, source_only=source_only, source=samples_source, mem_fea=mem_fea, mem_cls=mem_cls, class_weight_src=class_weight_src, img_idx=img_idx, source_idx=source_idx)
    result_list += total_loss
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='office_home',
                        choices=['office31', 'office_home', 'VisDA', 'domainnet','office10','PACS'], help='dataset used')
    parser.add_argument('--data-root-path', type=str, default='/root/PMTrans-main/dataset/', help='path to dataset txt files')
...
dataSubdata = {'PACS':['art_painting','cartoon','photo','sketch'],\
               'office31':['amazon','dslr','webcam'],
               'office10':['amazon','Caltech','dslr','webcam'],
               'OfficeHome':['Art','Clipart','Product','RealWorld'],
}
def loadData(config):
    ans = []
    isDel = False
    for i in range(len(dataSubdata[config.DATA.DATASET])):
        if dataSubdata[config.DATA.DATASET][i] == config.DATA.TARGET:
            continue
        config.DATA.SOURCE = dataSubdata[config.DATA.DATASET][i]
        dsets, dset_loaders = build_loader(config)
        if isDel:
            del dsets['target_train'],dsets['target_val'],dset_loaders['target_train'],dset_loaders['target_val']
        else:
            isDel = True
        ans.append(dset_loaders)
    return ans
def main(config):
    # dsets, dset_loaders = build_loader(config)
    # config.DATA.SOURCE = "Clipart"
    # dsets1, dset_loaders1 = build_loader(config)
    # config.DATA.SOURCE = "Art"
    # dsets2, dset_loaders2 = build_loader(config)
    # del dsets1['target_train'],dsets1['target_val'],dset_loaders1['target_train'],dset_loaders1['target_val']
    # del dsets2['target_train'],dsets2['target_val'],dset_loaders2['target_train'],dset_loaders2['target_val']
    dset_loader = loadData(config)
    config.defrost()
    # config.TRAIN.MAX_ITER = config.TRAIN.EPOCHS * max(len(dset_loaders['source_train']),len(dset_loaders1['source_train']),len(dset_loaders2['source_train']),
    #                                                   len(dset_loaders['target_train']))
    num_steps = max([len(x['source_train']) for x in dset_loader])
    num_steps = max([num_steps,len(dset_loader[0]['target_train'])])
    config.TRAIN.MAX_ITER = config.TRAIN.EPOCHS * num_steps
    ...
    mem_fea = torch.rand(len(dset_loader[0]["target_train"].dataset), config.MODEL.NUM_FEATURES).cuda()
    mem_fea = mem_fea / torch.norm(mem_fea, p=2, dim=1, keepdim=True)
    mem_cls = torch.ones(len(dset_loader[0]["target_train"].dataset),
                         config.MODEL.NUM_CLASSES).cuda() / config.MODEL.NUM_CLASSES

    logger.info("==============>Start training....................")
    start_time = time.time()
    max_accuracy = 0
    # dset_loader = [dset_loaders,dset_loaders1,dset_loaders2]
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        start_e = time.time()
        for i in dset_loader:
            i['source_train'].sampler.set_epoch(epoch)
        dset_loader[0]['target_train'].sampler.set_epoch(epoch)

        # train_one_epoch(config, model, dset_loaders, optimizer, epoch, writer, mem_fea, mem_cls)
        train_one_epoch(config, model, dset_loader,optimizer, epoch, writer, mem_fea, mem_cls)

        # if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
        if epoch == (config.TRAIN.EPOCHS - 1):
            save_checkpoint(config, epoch, model_without_ddp, logger)

        acc1 = validate(dset_loader[0]['target_val'], model, config)
...
def train_one_epoch(config, model, dset_loader,optimizer, epoch, writer,mem_fea,mem_cls):
    ...
    iter_sources = [iter(x['source_train']) for x in dset_loader]

    iter_target = iter(dset_loader[0]['target_train'])
    for idx in tqdm(range(num_steps)):
        samples_sources = []
        labels_source = []
        for i in range(len(iter_sources)):
            try:
                samples_source, label_source = iter_sources[i].next()
            except:
                iter_sources[i] = iter(dset_loader[i]['source_train'])
                samples_source, label_source = iter_sources[i].next()
            samples_sources.append(samples_source)
            labels_source.append(label_source) 
        try:
        ...
        for i in range(len(samples_sources)):
            (samples_sources[i]).cuda(non_blocking=True)
            (labels_source[i]).cuda(non_blocking=True)

        samples_target = (samples_target).cuda(non_blocking=True)

        threads = []
        total_loss = torch.tensor([0.]).cuda(non_blocking=True)
        for source_idx in range(len(samples_sources)):
            thread = threading.Thread(target=run_model, args=(model, samples_target, labels_source[source_idx], \
            config.sourceOnly, samples_sources[source_idx], mem_fea, mem_cls, class_weight_src, img_idx, source_idx, total_loss))
            threads.append(thread)
            thread.start()
        for thread in threads:
            thread.join()
        total_loss = total_loss/len(samples_sources)
