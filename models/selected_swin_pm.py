...
def mixup_soft_ce(pred, targets, weight, lam,selectedOutput):
    """ mixed categorical cross-entropy loss
    """
    loss = torch.nn.CrossEntropyLoss(reduction='none')(pred, targets)
    loss = torch.sum(selectedOutput*lam* weight* loss) / (torch.sum(weight*lam).item())
...
def loss_derta_2(output,y,selection_out):
  sum_ = 0
  size = y.size()[0]
  ...
  for i in range(0,size):
    ...
    temp = tip * torch.log(output_temp+1e-8) #log后进行选择函数的相乘
    sum_ = sum_ + decrementSum(temp,y) #对其进行 NLLLoss 函数
    # print(sum_.item(),end=" ")
...
class PMTrans(nn.Module):
    ...
    def __init__(self, num_classes=65, model_name='swin_base'):
        ...
        # 源域的选择头
        self.selection_out_source = nn.Sequential(
            nn.Linear(self.feature_dim,3),
            nn.Sigmoid()
        )

        
        self.s_dist_alpha = [nn.Parameter(torch.Tensor([1])),nn.Parameter(torch.Tensor([1])),nn.Parameter(torch.Tensor([1]))]
        self.s_dist_beta = [nn.Parameter(torch.Tensor([1])),nn.Parameter(torch.Tensor([1])),nn.Parameter(torch.Tensor([1]))]

        self.super_ratio = [nn.Parameter(torch.Tensor([-2])),nn.Parameter(torch.Tensor([-2])),nn.Parameter(torch.Tensor([-2]))]
        self.unsuper_ratio = [nn.Parameter(torch.Tensor([-2])),nn.Parameter(torch.Tensor([-2])),nn.Parameter(torch.Tensor([-2]))]
    ...
    def mix_source_target(self,s_token,t_token,s_lambda,t_lambda,pred,infer_label,s_logits,t_logits,s_scores,t_scores,mem_fea,img_idx,mem_cls,weight_tgt, weight_src,selective_head_source):
        ...
        t_scores = (torch.ones(16,self.num_patch)/self.num_patch).cuda()
       ...
       # s_lambda = mix_lambda(s_lambda, t_lambda) # without attention map
        ...
        super_m_s_t_s_loss = mixup_soft_ce(m_s_t_pred, infer_label,weight_src, s_lambda,selective_head_source)
        unsuper_m_s_t_loss = mixup_soft_ce(m_s_t_pred,      pred  ,weight_tgt, t_lambda,selective_head_source)
        select_loss = (loss_derta_2(m_s_t_pred,infer_label,selective_head_source) + loss_derta_2(m_s_t_pred,pred,selective_head_source))/2
        label_space_loss  = (super_m_s_t_s_loss + unsuper_m_s_t_loss)/torch.sum(s_lambda + t_lambda)

        return feature_space_loss,(label_space_loss + select_loss)/2
    

    def forward(self, target, infer_label, source=None, mem_fea=None, mem_cls=None, class_weight_src=None,
                img_idx=None, source_only=False,source_idx=0):
        ...
        if self.training:
            ...
            #计算选择头损失
            x = self.selection(s_logits)
            out = x/10
            selective_head_source = self.selection_out_source(out)#源域选择头的输出
            # print(selective_head_source[:,source_idx])
            ...
            t_lambda = dists.Beta(softplus(self.s_dist_alpha[source_idx]), softplus(self.s_dist_beta[source_idx])).rsample((B, self.num_patch,)).to(device).squeeze(-1)
            s_lambda = 1 - t_lambda

            super_m_s_t_loss, unsuper_m_s_t_loss = self.mix_source_target(s_token,t_token,s_lambda,t_lambda,pred,infer_label,s_logits,t_logits,s_scores,t_scores,mem_fea,img_idx,mem_cls,weight_tgt,weight_src,selective_head_source[:,source_idx].unsqueeze(1))
            # super_m_s_t_loss, unsuper_m_s_t_loss = self.mix_source_target(s_token,t_token,s_lambda,t_lambda,i_lambda,pred,infer_label,s_logits,t_logits,s_scores,t_scores,mem_fea,img_idx,mem_cls,weight_tgt,weight_src,)
            total_loss = classifier_loss+softplus(self.super_ratio[source_idx].to(device))*super_m_s_t_loss + softplus(self.unsuper_ratio[source_idx].to(device)) * unsuper_m_s_t_loss
            return total_loss
        else:
