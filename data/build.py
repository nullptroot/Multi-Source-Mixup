...
def build_loader(config):
    ...
    print(f"local rank {config.LOCAL_RANK} / global rank {dist.get_rank()} successfully build train dataset {config.DATA.SOURCE}  {config.DATA.TARGET}")
...
def build_transform(is_train, config):
    ...
    if is_train:
        transform = transforms.Compose([
            ...
            # transforms.RandomRotation(0.5),#1
            # transforms.ColorJitter(brightness=1),#2
            # transforms.ColorJitter(hue=0.5),#3
            transforms.ToTensor(),
